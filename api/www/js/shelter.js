...
var Shelter = function(_token, _testingMockData) {
    ...
    /** The array where all date requests for this shelter are stored*/
    var dateRequests = undefined;
    /** The facebook token that identifies this shelter - used in back end calls */
    var token = _token;
    ...
    //
    // param:
    //   callback - The function to be executed upon successful data
    //              retrieval.
    ...
    // Makes appropriate call to database/API to update
    // status for the specified requestId. The update consists of a new
    // status, and a feedback message. If no feedback is provided, the request
    // feedback is set to an empty string
    //
    // param:
    //   requestId - an unique integer value that identifies the request to be
    //               updated in database
    //   newStatus - The status to be set for the identified request
    //   feedback - A string message to be set for the identified request
    ...
    //   parameter 'filterBy' == "pending", or an
    //   objects with status == 'A' | 'D' if  parameter 'filterBy' == "history".
    ...
    // in the returned array of objects with the following tree properties:
    //
    //  {
    //     "id": 187326,
    //     "dateTime": "tomorrow",
    ...
    //
    // param:
    //   data - is the unfiltered data array which usually contains more fieLds
    //          than we need for shelter dateRequests.
    // return:
    //   An ordered array containing date requests with limited properties
    //
    ...
    function isActive(requestDateTime) {
        ...
        // requestDateTime could be a string, so we parse in base 10, then compare
    ...
    function validateRequestProperties(r) {
        ...
        if (typeof(r.user.street) === "undefined" || r.user.street === null || r.user.street === "") {
            r.user.street = "None provided";
        }
        if (typeof(r.user.city) === "undefined" || r.user.city === null || r.user.city === "") {
            r.user.city = "None provided";
        }
        if (typeof(r.user.state) === "undefined" || r.user.state === null || r.user.state === "") {
            r.user.state = "None provided";
        }
        if (typeof(r.user.zip) === "undefined" || r.user.zip === null || r.user.zip === "") {
            r.user.zip = "None provided";
        }
        if (typeof(r.user.fname) === "undefined" || r.user.fname === null || r.user.fname === "") {
            r.user.fname = "None provided";
        }
        if (typeof(r.user.lname) === "undefined" || r.user.lname === null || r.user.lname === "") {
            r.user.lname = "None provided";
        }
        if (typeof(r.user.email) === "undefined" || r.user.email === null || r.user.email === "") {
            r.user.email = "None provided";
        }
        if (typeof(r.user.phone) === "undefined" || r.user.phone === null || r.user.phone === "") {
            r.user.phone = "None provided";
        }

        // Verify dog profile properties
        if (typeof(r.dog.id) === "undefined" || r.dog.id === null || r.dog.id === "") {
            r.dog.id = "None provided";
        }
        if (typeof(r.dog.name) === "undefined" || r.dog.name === null || r.dog.name === "") {
            r.dog.name = "None provided";
        }
        if (typeof(r.dog.sex) === "undefined" || r.dog.sex === null || r.dog.sex === "") {
            r.dog.sex = "None provided";
        }
        if (typeof(r.dog.age) === "undefined" || r.dog.age === null || r.dog.age === "") {
            r.dog.age = "None provided";
        }
        if (typeof(r.dog.media.photos[1]) === "undefined") {
            r.dog.media.photos[1] = "Non provided";
        }

        // Verify request properties
        if (typeof(r.request.id) === "undefined" || r.request.id === null || r.request.id === "") {
            r.request.id = "None provided";
        }
        if (typeof(r.request.status) === "undefined" || r.request.status === null || r.request.status === "") {
            r.request.status = "P";
        }
        if (typeof(r.request.epoch) === "undefined" || r.request.epoch === null || r.request.epoch === "") {
            r.request.epoch = "None provided";
        }
        if (typeof(r.request.reason) === "undefined" || r.request.reason === null || r.request.reason === "") {
            r.request.reason = "None provided";
        }
        if (typeof(r.request.feedback) === "undefined" || r.request.feedback === "") {
            r.request.feedback = "None provided";
        }
        return r;
    }
