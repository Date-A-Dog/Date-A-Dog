...
class FindZipcodes(object):
    ...
    def get_coordinates(self):
        ...
        (latitude, longitude)
        """
        query = "SELECT latitude, longitude FROM zipcodes WHERE zipcode = '" \
           + str(self.zipcode) + "'"
    ...
    def get_bounding_coords(self, lat, lon):
        """Get bounding coordinates; calculates great circle for longitude, 
        providing more accurate results than if we were to use the circle of 
        latitude as reference. (Doesn't account for anomalies at the poles or 
        along the 180th meridian--fine for US Zipcodes, but another application 
        might require an adjustment. Returns a list of the bounding coordinates.
        """
    ...
    def get_zipcodes(self):
        """Get coordinates for request zipcode, then calculate directional 
        bounds based on distance. Based on the bounding coordinates, return 
        a list of zipcodes within the bound, in order of geodesic distance.
        """
        ...
        query = "SELECT * FROM zipcodes WHERE latitude BETWEEN " + \
          str(bounds[0]) + " AND " + str(bounds[1]) + \
          " AND longitude BETWEEN " + str(bounds[2]) + " AND " + \
          str(bounds[3]) 
